  ...
  def initialize
    @parser = Parser.new('Flip/Flop') do
      ...
      rule :if_statement do
        ...
        # If
        match('fi', '(', :expression, ')', :statement_list, 'if') {
          |_, _, expressions, _, stmt_list, _|
      ...
      rule :expression_addition do
        ...
        match(:expression_addition, '+', :expression_multiplication) {
          |expr1, operator, expr2|
          Compound_Node.new(operator, expr1, expr2)
        }
        match(:expression_addition, '-', :expression_multiplication) {
      ...
      rule :expression_multiplication do
        ...
        match(:expression_multiplication, '*', :expression_unary) {
          |expr1, operator, expr2|
          Compound_Node.new(operator, expr1, expr2)
        }
        match(:expression_multiplication, '/', :expression_unary) {
      ...
      rule :comparison do
        match(:expression_addition, :op_relational, :expression_addition) {
          |expr1, operator, expr2|
          Compound_Node.new(operator, expr1, expr2)
        }
        match(:expression_addition) {
          |expr|
      ...
      rule :function_declare do
        match('boj', :statement_list, 'job', :identifier, '(', :parameter_list, ')') {
          |_, stmt_list, _, name, _, param_list, _|
          FunctionDeclare_Node.new(stmt_list, name, param_list)
        }
        match('boj', :statement_list, 'job', :identifier, '(', ')') {
          |_, stmt_list, _, identifier, _, _|
